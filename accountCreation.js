...
prompt.get([{
  ...
  request.post({url: endpoint, formData: formData}, function optionalCallback(err, httpResponse, body) {
    ...
    if (addressid == null || creditcardid == null) {
      if (addressid == null) {
        prompt.get([{
            name:'first',
            description: 'Enter your First Name',
          ...
          var endpoint = baseURL + '/user/' + userid + '/address/new';
      ...
      if (creditcardid == null) {
        console.log('Consult the braintree documentation for more information on how to tokenize credit cards to get a payment method nonce.\nhttps://developers.braintreepayments.com');
        prompt.get([{
            name:'cardholder_name',
            description: 'Enter the Card Holder\'s Name',
            required: true
          }, {
          ...
          var endpoint = baseURL + '/user/' + userid + '/credit_cards/new';
          ...
          request.post({url: endpoint, formData: formData}, function optionalCallback(err, httpResponse, body) {
            ...
            creditcardid = jsonBody.address.address_id;
        });
        });
      }//second if
